////////////////////////////////////////////////////////////////////////////////
// Filename: CSkydomeShaderclass.cpp
////////////////////////////////////////////////////////////////////////////////
...
bool CSkydomeShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, "..\\..\\source\\Skydome.vs", "..\\..\\source\\Skydome.ps");
	if(!result)
	{
		return false;
	}
...
bool CSkydomeShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
...
bool CSkydomeShader::InitializeShader(ID3D11Device* device, HWND hwnd, char * vsFilename, char * psFilename)
{
	HRESULT result;
	...
	result = D3DX11CreateShaderResourceViewFromFile(device, "..\\..\\data\\Sky_horiz_6-2048.jpg", NULL, NULL, &m_skydomeTexture, NULL);

	if(FAILED(result))
	{
		MessageBox(hwnd, "Could create resource from Skydome03.jpg in CSkydomeShader.cpp line 89", "Error", MB_OK);
		return false;
	}
...
void CSkydomeShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, char * shaderFilename)
{
	char* compileErrors;
...
bool CSkydomeShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
											 D3DXMATRIX projectionMatrix, D3DXVECTOR4 apexColor, D3DXVECTOR4 centerColor)
{
	HRESULT result;
	...
	result = deviceContext->Map(m_gradientBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource2);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr2 = (GradientBufferType*)mappedResource2.pData;

	// Copy the gradient color variables into the constant buffer.
	dataPtr2->apexColor = apexColor;
	dataPtr2->centerColor = centerColor;
	dataPtr2->ambientColor = D3DXVECTOR4(1.0f, 0.0f, 1.0f, 1.0f);
	dataPtr2->diffuseColor = D3DXVECTOR4(.5f, .2f, 0.0f, 1.0f);
	dataPtr2->lightDirection = D3DXVECTOR3(-0.5f, -1.0f, 0.0f);
	...
	/*
	// Get a pointer to the data in the constant buffer.
	dataPtr3 = (LightBufferType*)mappedResource3.pData;


	// Copy the lighting variables into the constant buffer.
	dataPtr3->ambientColor = D3DXVECTOR4(1.0f, 0.0f, 1.0f, 1.0f);
	dataPtr3->diffuseColor = D3DXVECTOR4(.5f, .2f, 0.0f, 1.0f);
	dataPtr3->lightDirection = D3DXVECTOR3(-0.5f, -1.0f, 0.0f);
	...
	temp[0] = m_gradientBuffer;
	temp[1] = m_lightBuffer;
	
	// Finally set the light constant buffer in the pixel shader with the updated values.
	deviceContext->PSSetConstantBuffers(bufferNumber, 2, temp);// &m_lightBuffer);
*/
	...
	deviceContext->PSSetShaderResources(0, 1, &m_skydomeTexture);
