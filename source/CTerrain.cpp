...
bool CTerrain::Initialize(ID3D11Device* device, char* heightMapFilename, char * textureFilename, char* colorMapFilename)
{
	bool result;
...
/*
void CTerrain::Render(ID3D11DeviceContext* deviceContext, XMMATRIX& vPMatrix)
{
	// Put the vertex and index buffers on the graphics pipeline to prepare them for drawing.
	RenderBuffers(deviceContext, vPMatrix);
...
ID3D11ShaderResourceView* CTerrain::GetTexture()
{
...
bool CTerrain::LoadHeightMap(char* filename)
{
	...
	CPerlinNoise *pn = new CPerlinNoise(4.2f, .0612f, .7f, 4, 8);
	...
	m_terrainWidth = bitmapInfoHeader.biWidth/8;
	m_terrainHeight = bitmapInfoHeader.biHeight/8;
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].x = (float)i * TERRAIN_CELL_WIDTH;
			m_heightMap[index].z = (float)j * TERRAIN_CELL_WIDTH;
			m_heightMap[index].y = (float)pn->GetHeight(i * TERRAIN_CELL_WIDTH, j * TERRAIN_CELL_WIDTH) * HEIGHT_MULTIPLIER;
...
void CTerrain::ShutdownHeightMap()
{
	if(m_heightMap)
	{
		delete [] m_heightMap;
...
bool CTerrain::LoadTexture(ID3D11Device* device, char * filename)
{
	bool result;
...
bool CTerrain::LoadColorMap(char* filename)
{
	...
	for(j=0; j<colorMapHeight; j++)
	{
		for(i=0; i<colorMapWidth; i++)
		{
			...
			m_heightMap[index].g = (float)(bitmapImage[k+1] / 255.0f)+.2;
			m_heightMap[index].r = (float)(bitmapImage[k+2] / 255.0f)+.5;		// Orange Sky Reflection
...
bool CTerrain::InitializeBuffers(ID3D11Device* device)
{
	//VertexType* vertices;
...
float CTerrain::GetHeightAtPointXY(float x, float z)
{
	float newY;
	D3DXVECTOR3 N, R, P, Q;

	
	if( ( (x - 1) > 0 && (x + 1) < m_terrainHeight * TERRAIN_CELL_WIDTH ) && ( (z - 1) > 0 && (z + 1) < m_terrainWidth * TERRAIN_CELL_WIDTH ) )
	{
		R = D3DXVECTOR3(m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)) + (int)(x/TERRAIN_CELL_WIDTH)].x, 
			m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)) + (int)(x/TERRAIN_CELL_WIDTH)].y, 
			m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)) + (int)(x/TERRAIN_CELL_WIDTH)].z);

		P = D3DXVECTOR3(m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)+1) + (int)(x/TERRAIN_CELL_WIDTH)+1].x, 
			m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)+1) + (int)(x/TERRAIN_CELL_WIDTH)+1].y, 
			m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)+1) + (int)(x/TERRAIN_CELL_WIDTH)+1].z);

		Q = D3DXVECTOR3(m_heightMap[(m_terrainHeight * (int)z/TERRAIN_CELL_WIDTH-1) + (int)(x/TERRAIN_CELL_WIDTH)-1].x, 
			m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)-1) + (int)(x/TERRAIN_CELL_WIDTH)-1].y, 
			m_heightMap[(m_terrainHeight * (int)(z/TERRAIN_CELL_WIDTH)-1) + (int)(x/TERRAIN_CELL_WIDTH)-1].z);
		...
		N = D3DXVECTOR3(A.z*B.y - A.y*B.z, A.x*B.y - A.y*B.x, A.x*B.z - A.z*B.x);

		newY = ( ( N.x * ( x - P.x ) + N.z * (  z - P.z ) / -N.y) ) + P.y;		//  Per-pixel interpolation -- Need "Point Soup" to no jump

		//newY = (R.y + P.y + Q.y) / 3;											//  Simple 3 Point random selection
...
CTerrain::HeightMapType *CTerrain::GetHeightMap()
{
	return m_heightMap;
}
...
void CTerrain::CopyVertexArray(void* vertexList)
{
