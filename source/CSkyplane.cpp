...
CSkyplane::CSkyplane(const CSkyplane& other)
{
}
...
bool CSkyplane::Initialize(ID3D11Device* device, char * cloudTextureFilename, char * perturbTextureFilename)
{
...
void CSkyplane::Render(ID3D11DeviceContext* deviceContext)
{
...
int CSkyplane::GetIndexCount()
{
	return m_indexCount;
}


ID3D11ShaderResourceView* CSkyplane::GetCloudTexture()
{
	return m_CloudTexture->GetTexture();
}


ID3D11ShaderResourceView* CSkyplane::GetPerturbTexture()
{
	return m_PerturbTexture->GetTexture();
}


float CSkyplane::GetScale()
{
	return m_scale;
}


float CSkyplane::GetBrightness()
{
...
float CSkyplane::GetTranslation()
{
...
bool CSkyplane::InitializeSkyPlane(int skyPlaneResolution, float skyPlaneWidth, float skyPlaneTop, float skyPlaneBottom, int textureRepeat)
{
...
void CSkyplane::ShutdownSkyPlane()
{
	// Release the sky plane array.
	if(m_skyPlane)
	{
...
bool CSkyplane::InitializeBuffers(ID3D11Device* device, int skyPlaneResolution)
{
	VertexType* vertices;
...
void CSkyplane::ShutdownBuffers()
{
...
void CSkyplane::RenderBuffers(ID3D11DeviceContext* deviceContext)
{
	unsigned int stride;
	unsigned int offset;
...
bool CSkyplane::LoadTextures(ID3D11Device* device, char * textureFilename1, char * textureFilename2)
{
	bool result;


	// Create the cloud texture object.
	m_CloudTexture = new CTexture;
	if(!m_CloudTexture)
	{
		return false;
	}
...
void CSkyplane::ReleaseTextures()
{
