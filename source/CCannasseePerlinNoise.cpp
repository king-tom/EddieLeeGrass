...
class OptimizedPerlin {
  ...
  std::vector<float> aOctFreq; // frequency per octave
  std::vector<float> aOctPers; // persistence per octave
  float fPersMax;// 1 / max persistence
  ...
  void new1( ?seed, ?octaves, ?falloff ) // Stopped porting
  {
