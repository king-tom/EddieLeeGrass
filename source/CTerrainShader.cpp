...
bool CTerrainShader::Initialize( ID3D11Device* device, HWND hwnd )
{
	...
	result = InitializeShader( device, hwnd, "..\\..\\source\\Terrain.vs", "..\\..\\source\\Terrain.ps" );
	if( !result )
	{
		return false;
	}
...
bool CTerrainShader::Render( ID3D11DeviceContext* deviceContext, 
							int indexCount, 
...
bool CTerrainShader::InitializeShader( ID3D11Device* device, HWND hwnd, char * vsFilename, char * psFilename )
{
	HRESULT result;
...
void CTerrainShader::OutputShaderErrorMessage( ID3D10Blob* errorMessage, 
											  HWND hwnd, 
											  char * shaderFilename )
{
	char* compileErrors;
...
bool CTerrainShader::SetShaderParameters( ID3D11DeviceContext* deviceContext,
										 D3DXMATRIX worldMatrix, 
										 D3DXMATRIX viewMatrix, 
										 D3DXMATRIX projectionMatrix, 
										 D3DXVECTOR4 ambientColor, 
										 ...
										 D3DXVECTOR2 cameraLocationXZ )
{
	...
	//cameraDirection += D3DXVECTOR3( .05f, .05f, .05f );
...
void CTerrainShader::ToggleWireFrame( ID3D11Device* device )			//TODO: change to SetFillMode
{
