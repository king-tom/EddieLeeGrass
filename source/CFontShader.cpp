...
bool CFontShader::Initialize( ID3D11Device* device, HWND hwnd )
{
	...
	result = InitializeShader( device, hwnd, "..\\..\\source\\Font.vs", "..\\..\\source\\Font.ps" );
	if(!result)
	{
		return false;
	}
...
bool CFontShader::Render( ID3D11DeviceContext* deviceContext, 
						 int indexCount, 
...
bool CFontShader::InitializeShader( ID3D11Device* device, HWND hwnd, char * vsFilename, char * psFilename )
{
	HRESULT result;
...
void CFontShader::OutputShaderErrorMessage( ID3D10Blob* errorMessage, HWND hwnd, char * shaderFilename )
{
	char* compileErrors;
